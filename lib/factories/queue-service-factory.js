...
module.exports = class QueueServiceFactory {
    ...
    getTopicQueueService() {
        try {
            switch (this.__config.queueProvider) {
                case 'memory':
                    return require('../services/queues/topic/memory-queue-service').create(this.__config, this.__logger);
                case 'rabbitmq':
                    return require('../services/queues/topic/rabbit-queue-service').create(this.__config, this.__logger, this.__coreRabbitService);
                default:
                    return null;
            }
        } catch (err) {
