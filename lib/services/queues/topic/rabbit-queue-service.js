...
module.exports = class RabbitQueueService {
    ...
    static create(config, logger, amqpClient) {
        return new RabbitQueueService(config, logger, amqpClient);
    }
    ...
    startExchange(exchangeName) {
        ...
        this.__channel.assertExchange(exchangeName, 'topic', {
            durable: this.__config.durable
        });

        // this.__channel.prefetch(1);

        this.__emitter.emit('exchangeStarted', {});
    }

    publish(exchange, key, item) {
        let msg = (typeof item !== 'string') ? JSON.stringify(item) : item;
        this.__channel.publish(exchange, key, Buffer.from(msg));
        this.__emitter.emit('itemAdded', { exchange, item });
    }
