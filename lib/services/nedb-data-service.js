...
module.exports = class NedbDataService {
    ...
    async processStore(message) {
        ...
        this.__logger.debug('processStore invoked: ', message);
        ...
        try {
            ...
            this.__logger.error('processStore', err);
            throw err;
        }
    }
    ...
    async upsert(msgObj, dataWasMerged) {

        let self = this;

        try {
            this.__logger.debug('upsert request object ', msgObj);

            let upsertData = self.__prepareUpsertData(msgObj);
            let upsertResult = await self.__update({ _id: upsertData.path }, upsertData.clonedParams.$set, upsertData.options);
            self.__updateMeta(upsertResult, upsertData);
            let transformed = self.__transform(upsertResult, upsertData);
            transformed.response = upsertResult.response;

            this.__logger.debug('tramsformed upsert result: ', transformed);
    ...
    __prepareUpsertData(msgObj) {
        ...
        // TODO: WITHOUT THIS, IF YOU REPLAY THE SAME 'SET' REQUEST, YOU WILL GET UNIQUE CONSTRAINT VIOLATION ERROR
        // if (options.merge)...
